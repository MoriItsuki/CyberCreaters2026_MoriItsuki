...
//	静的メンバ変数初期化
//========================================================
int CGaugeManager::m_Cnt = 0;	//ダッシュ可能数
LPD3DXFONT CGaugeManager::m_pFont = nullptr;
...
HRESULT CDashgauge::Init()
{
	...
	try
	{
		m_pArcPart = make_unique<CArcPart>(
			ARC_GAUGE::ARCRADIUS,							//	円の半径
			270.0f,							//	円最大角度(値がMAXのときの角度)
			D3DCOLOR_XRGB(255, 0, 0),		//	始まりの色
			D3DCOLOR_XRGB(255, 255, 0),		//	終わりの色
			vec3(ARC_GAUGE::ARCRADIUS + 20.0f, CProcess::SCREEN_HEIGHT - (ARC_GAUGE::ARCRADIUS + 20.0f), 0.0f)		//	画面上の位置(ｚは"0.0"固定)
			);
...
//	生成処理
//========================================================
CDashgauge* CDashgauge::Create(float radius, float maxAngle, D3DCOLOR startColor, D3DCOLOR endColor, const D3DXVECTOR3& pos)
{
	CDashgauge* pGauge = new CDashgauge();
	pGauge->Init();
	pGauge->GetArcGauge()->SetRadius(radius);
	pGauge->GetArcGauge()->SetMaxAngle(maxAngle);
	pGauge->GetArcGauge()->SetColors(startColor, endColor);
	pGauge->GetArcGauge()->SetPos(pos);
...
HRESULT CArcPart::Init()
{
	...
	UINT vertexSize = static_cast<UINT>(sizeof(VERTEX) * (ARC_GAUGE::SEGMENT + 2));	//中心点+セグメント分
...
{
	// ゲージの値を更新
	float currentAngle = D3DXToRadian(m_maxAngle * GetValue());
	vector<vec2> vertices2D = GenerateArcVertices(m_radius, currentAngle, ARC_GAUGE::SEGMENT);
...
//	ゲージの位置設定
//========================================================
void CArcPart::SetPos(const D3DXVECTOR3& pos)
{
	m_centerPos = pos;
}
...
//	ゲージの半径設定
//========================================================
void CArcPart::SetRadius(float radius)
{
	m_radius = radius;
}
...
//	ゲージの最大角度の設定
//========================================================
void CArcPart::SetMaxAngle(float angle)
{
	m_maxAngle = angle;
}
...
{
	vector<vec2> vertices2D = GenerateBarVertices(m_length * GetValue(), m_thickness);
...
namespace STRAIGHTGAUGE_GAUGE
{
	constexpr float SIZE_X = 100.0f;	//ゲージの横サイズ
	constexpr float SIZE_Y = 100.0f;	//ゲージの縦サイズ

	constexpr float MAXSIZE_X = 200.0f;	//	最大横幅
}
...
//	静的メンバ変数初期化
//
const std::string CGauge2D::FILE_STRAIGHTGAUGE = "data\\texture\\.png";
...
HRESULT CGauge2D::Init() 
{
	...
	m_size = vec3(STRAIGHTGAUGE_GAUGE::SIZE_X, STRAIGHTGAUGE_GAUGE::SIZE_Y, 0.0f);
	m_color = D3DCOLOR_XRGB(0, 0, 0);

	//	テクスチャ作成
	CTexture* pTex = CManager::GetTexture();
	BindTexture(pTex->GetAddress(pTex->Regist(FILE_STRAIGHTGAUGE)));
...
{
	// ゲージの長さを計算（現在値を最大値で割った比率）
	float ratio = m_currentValue / m_maxValue;

	// ゲージの幅を調整
	m_size.x = STRAIGHTGAUGE_GAUGE::MAXSIZE_X * ratio; // 割合調整
...
//	最大値設定
//
void CGauge2D::SetMaxValue(float maxValue) 
{
	m_maxValue = maxValue;
}
...
//	現在値設定
//
void CGauge2D::SetCurrentValue(float value)
{
	m_currentValue = clamp(value, 0.0f, m_maxValue);
}

//
//	生成処理
//
CGauge2D* CGauge2D::Create(float maxvalue, float value, D3DXVECTOR3 pos, D3DXCOLOR color)
{
	CGauge2D* pGauge = new CGauge2D();

	pGauge->Init();
	pGauge->SetMaxValue(maxvalue);
	pGauge->SetCurrentValue(value);
	pGauge->SetPos(pos);
